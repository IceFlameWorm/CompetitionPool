    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options, allow_soft_placement=True)) as sess:
        ...
        try:
            ...
            while not coord.should_stop():
                ...
                _, loss_val, train_summary, step = sess.run([graph['train_op'], graph['loss'],
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options,allow_soft_placement=True)) as sess:
        ...
        try:
            ...
            while not coord.should_stop():
                ...
                acc_top_k += acc_k
                end_time = time.time()
                logger.info("the batch {0} takes {1} seconds, accuracy = {2}(top_1) {3}(top_k)".format(i, end_time - start_time, acc_1, acc_k))
        except tf.errors.OutOfRangeError:
